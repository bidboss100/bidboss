          ...
          return (
            <div key={stage} style={{ minWidth: '240px', width: '240px', flexShrink: 0 }}>
              ...
              <div style={{
                ...
                <p style={{ fontSize: '0.62rem', color: '#6C757D', marginTop: '2px' }}>{fmt(stageValue)}</p>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {cards.map(opp => (
                  <div key={opp.id} style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                      <span style={{ fontSize: '0.58rem', color: '#6C757D' }}>{opp.solicitation}</span>
                      <ScoreBadge score={opp.aiScore} />
                    </div>
                    <p style={{ color: '#1A1A1A', marginBottom: '4px', lineHeight: 1.3 }}>
                      {opp.title.length > 45 ? opp.title.slice(0, 45) + '…' : opp.title}
                    </p>
                    <p style={{ color: '#6C757D', fontSize: '0.62rem', marginBottom: '6px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginTop: '6px', fontSize: '0.6rem', color: '#6C757D' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.4rem', marginTop: '8px' }}>
                      {STAGES.indexOf(stage) > 0 && (
                        <button onClick={() => move(opp.id, -1)} style={{
                          flex: 1, padding: '3px', background: 'transparent', border: '1px solid #DEE2E6',
                          borderRadius: '4px', color: '#6C757D', fontSize: '0.6rem', cursor: 'pointer',
                ...
                {cards.length === 0 && (
                  <div style={{
                    border: '1px dashed #DEE2E6', borderRadius: '8px', padding: '1.5rem',
                    textAlign: 'center', color: '#ADB5BD', fontSize: '0.68rem',
                  }}>
                    No opportunities
                  </div>
                )}
